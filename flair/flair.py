...
from flair.utilities import get_tool_path, subprocess_wrapper
import flair.sam_to_psl as sam_to_psl
from flair.bam2Bed12 import bamToBed12
...
def parse_align():
    ...
    required.add_argument('-r', '--reads', action='store', dest='r', nargs='+', type=str, required=True,
                          help='FastA/FastQ files of raw reads')
    required.add_argument('-g', '--genome', action='store', dest='g', type=str, required=True,
                          help='FastA of reference genome, can be minimap2 indexed')
    parser.add_argument('-m', '--minimap2', type=str, default='minimap2', action='store', dest='m',
                        help='path to minimap2 if not in $PATH')
    ...
    parser.add_argument('--psl', action='store_true', dest='p', help='also output sam-converted psl')
    ...
    if unknown:
        print(f"Align unrecognized arguments: {' '.join(unknown)}", file=sys.stderr)
...
def align(genome_file, reads_list, output_dir, make_psl=False, chromosome_size=None, quality=0, version=None, prefix="flair.aligned",
    ...
    if quiet:
        error_out = open(os.devnull, "w")
    else:
        error_out = sys.stderr

    output_path = str(output_dir + prefix)
    # run minimap2 alignment to reference genome

    nvrna_options = ['-uf', '-k14'] if nvrna else []
    mm2_command = [minimap2_path, '-ax', 'splice', '-t', threads] + nvrna_options + \
                  ['--secondary=no', genome_file] + reads_list
    mm2_stderr = Path(output_path + ".mm2_stderr") if quiet else None
    sam_align_file = Path(output_path + ".sam")
    subprocess_wrapper(mm2_command, sam_align_file, mm2_stderr)

    # Filter the alignments if a quality score is given
    if quality != 0:
        sam_command = [samtools_path, 'view', '-q', str(quality), '-h', '-S', sam_align_file]
        sam_quality_filtered_file = Path(output_path + '.q.sam')
        sam_filter_stderr = Path(output_path + '.samtools_filter_stderr') if quiet else None
        subprocess_wrapper(sam_command, sam_quality_filtered_file, sam_filter_stderr)
        # Overwrite the original sam file with the filtered one
        # This should use the os.move or whatever to be more portable
        os.replace(sam_quality_filtered_file, sam_align_file)
        os.remove(sam_filter_stderr)

    # create psl if requested
    if make_psl:
        sam_to_psl.psl_converter(sam_align_file, output_path + ".psl", chromosome_size)

    # convert sam to bam file
    unsorted_bam_file = Path(output_path + '.unsorted.bam')
    sam_view_stderr = Path(output_path + '.samtools_view_stderr') if quiet else None
    subprocess_wrapper([samtools_path, 'view', '-h', '-Sb', '-@', threads, sam_align_file],
                       unsorted_bam_file, sam_view_stderr)

    # sort the bam file, if samtools <1.3 "-o" flag is required
    use_dash_o = False
    try:
        if version is None:  # samtools version is < 1.3 or unspecified --> detect version
            version = subprocess.run([samtools_path, "--version"], stdout=subprocess.PIPE, check=True)
            version = float(version.stdout.split()[1])
            print(f"Samtools version {version} detected", file=error_out)
        if version >= 1.3:
            use_dash_o = True
    except subprocess.CalledProcessError:
        print('Could not determine samtools version, assuming < 1.3', file=error_out)

    bam_out_flag = ["-o"] if use_dash_o else []
    sorted_bam_file = Path(output_path + '.bam')
    sam_sort_stderr = Path(output_path + '.unsorted.bam.stderr') if quiet else None
    sam_sort_command = [samtools_path, 'sort', '-@', threads, unsorted_bam_file] + bam_out_flag + [sorted_bam_file]
    subprocess_wrapper(sam_sort_command, stdout_file=None, stderr_file=sam_sort_stderr)
    print('If using samtools v1.3+, please specify -v1.3 argument', file=error_out)

    # Index the bam file
    subprocess_wrapper([samtools_path, "index", sorted_bam_file],
                       stdout_file=None,
                       stderr_file=Path(output_path + 'index.bam.stderr') if quiet else None)

    align_bed = Path(output_path + ".bed")
    with open(align_bed, "w") as out_file:
        [out_file.write("\t".join(record)) for record in bamToBed12(sorted_bam_file)]

    # Do some clean up of files/etc
    [os.remove(file) for file in [unsorted_bam_file, sam_view_stderr, sam_sort_stderr] if file is not None]
    if quiet:
        error_out.close()
...
def parse_correct(aligned_reads=''):
    parser = argparse.ArgumentParser(description='flair-correct parse options',
                                     usage='python flair.py correct -q query.bed12 [-f annotation.gtf] [-j introns.tab]'
                                           ' -g genome.fa [options]')
    ...
    if not aligned_reads:
        required.add_argument('-q', '--query', type=str, default='', required=True, action='store', dest='q',
    ...
    atleastone.add_argument('-f', '--gtf', default='', action='store', dest='f', help='GTF annotation file')
    parser.add_argument('-c', '--chromsizes', type=str, action='store', dest='c', default='',
                        help='chromosome sizes tab-separated file')
    ...
    parser.add_argument('-o', '--output', action='store', dest='o', default='./',
                        help='output name base (default: cwd)')
    parser.add_argument('--print_check', action='store_true', dest='p', default=False,
    ...
    sources = [("-j", args.j)] if args.j else []
    sources += [("-f", args.f)] if args.f else []

    return correct(aligned_reads, args.g, sources, args.o,
                   chromosome_sizes=args.c, windows_size=args.w, nvrna=args.n, threads=args.t, print_check=args.p)


def correct(raw_alignment, genome_file, correction_sources, output_dir,
            chromosome_sizes=None, windows_size=10, nvrna=False, threads=4, prefix="flair", print_check=False):
    """
    Correct long read alignment.
    correction_sources: list of tuples ("-f|-j", file_path) where -j is shortread inputs and -f is for gtf inputs
    """
    # TODO: this whole thing needs better error handling
    output_path = str(output_dir + prefix)
    # TODO: rewrite and import ssCorrect.py
    correction_cmd = [sys.executable, path + 'bin/ssCorrect.py', '-i', raw_alignment, '-w', windows_size, '-p', threads,
                      '-o', output_path, '--progress', '-f', genome_file]
    correction_cmd += ['--correctStrand'] if nvrna else []
    for correction_source in correction_sources:
        correction_cmd += correction_source
    if print_check:
        correction_cmd += ['--print_check']

    subprocess_wrapper(correction_cmd, None)

    # TODO: rewrite and import bed_to_psl.py
    if chromosome_sizes is not None:
        subprocess_wrapper([sys.executable, path + 'bin/bed_to_psl.py', chromosome_sizes,
                            output_path + '_all_corrected.bed', output_path + '_all_corrected.psl'], None)

    return output_path + '_all_corrected.bed'


def collapse_range(corrected_reads='', aligned_reads=''):
    parser = argparse.ArgumentParser(description='flair-collapse parse options',
                                     usage='python flair.py collapse-range -g genome.fa -r reads.bam '
                                           '-q <query.psl>|<query.bed> [options]')
    parser.add_argument('collapse')
    required = parser.add_argument_group('required named arguments')
    required.add_argument('-r', '--reads', action='store', dest='r', nargs='+', type=str, required=True,
                          help='bam file(s) of the aligned reads')
    if not corrected_reads:
        required.add_argument('-q', '--query', type=str, default='', required=True, action='store', dest='q',
                              help='bed or psl file of aligned/corrected reads')
    required.add_argument('-g', '--genome', action='store', dest='g', type=str, required=True,
                          help='FastA of reference genome')
    parser.add_argument('-f', '--gtf', default='', action='store', dest='f',
                        help='GTF annotation file, used for renaming isoforms from annotations and adjusting TSS/TESs')
    parser.add_argument('-m', '--minimap2', type=str, default='minimap2', action='store', dest='m',
                        help='path to minimap2 if not in $PATH')
    ...
    parser.add_argument('-b', '--bedtools', action='store', dest='b', default='bedtools',
                        help='bedtools path, provide if promoter regions specified and bedtools is not in $PATH')
    parser.add_argument('-sam', '--samtools', action='store', dest='sam', default='samtools',
                        help='samtools executable path if not in $PATH')
    ...
    parser.add_argument('--stringent', default=False, action='store_true', dest='stringent',
                        help='specify if all supporting reads need to be full-length (80%% coverage and spanning 25 bp'
    ...
    parser.add_argument('-i', '--isoformtss', default=False, action='store_true', dest='i',
                        help='when specified, TSS/TES for each isoform will be determined from supporting reads for '
                             'individual isoforms (default: not specified, determined at the gene level)')
    ...
    parser.add_argument('--keep_intermediate', default=False, action='store_true', dest='keep_intermediate',
                        help='specify if intermediate and temporary files are to be kept for debugging. Intermediate '
                             'files include: promoter-supported reads file, read assignments to firstpass isoforms')
    parser.add_argument('--generate_map', default=False, action='store_true', dest='generate_map',
                        help='specify this argument to generate a txt file of which reads are assigned to each isoform '
                             'note: This does not work with salmon for quantification (default: not specified)')
    parser.add_argument('--quiet', default=False, action='store_true', dest='quiet',
                        help='''Suppress progress statements from being printed''')
    parser.add_argument('--salmon', type=str, action='store', dest='salmon', default='',
                        help='Path to salmon executable, specify if salmon quantification is desired')
    ...
    parser.add_argument('-o', '--output', default='flair.collapse', action='store', dest='o',
    ...
    if subprocess.call(['bedPartition', '-parallel=' + str(args.t), args.temp_dir + run_id + '.sorted.bed',
                        args.o + '.ranges.bed']):
    ...
    subprocess.call(['cat'] + glob.glob(args.temp_dir + run_id + '*isoforms.bed'),
                    stdout=open(args.o + '.isoforms.bed', 'w'))
    subprocess.call(['cat'] + glob.glob(args.temp_dir + run_id + '*isoforms.fa'),
                    stdout=open(args.o + '.isoforms.fa', 'w'))
    if args.f:
        subprocess.call(['cat'] + glob.glob(args.temp_dir + run_id + '*isoforms.gtf'),
...
def collapse(genomic_range='', corrected_reads=''):
    ...
    required.add_argument('-r', '--reads', action='store', dest='r', nargs='+', type=str, required=True,
                          help='FastA/FastQ files of raw reads')
    parser.add_argument('-f', '--gtf', default='', action='store', dest='f',
                        help='GTF annotation file, used for renaming FLAIR isoforms to annotated isoforms and adjusting'
                             ' TSS/TESs')
    parser.add_argument('-m', '--minimap2', type=str, default='minimap2', action='store', dest='m',
                        help='path to minimap2 if not in $PATH')
    parser.add_argument('-t', '--threads', type=int, action='store', dest='t', default=4,
    ...
    parser.add_argument('-b', '--bedtools', action='store', dest='b', default='bedtools',
                        help='bedtools executable path, provide if TSS/TES regions specified and bedtools is not in'
                             ' $PATH')
    parser.add_argument('-sam', '--samtools', action='store', dest='sam', default='samtools',
                        help='samtools executable path if not in $PATH')
    parser.add_argument('-w', '--end_window', default='100', action='store', dest='w',
    ...
    parser.add_argument('--no_end_adjustment', default=False, action='store_true', dest='no_end_adjustment', help='''when specified, TSS/TES from the gtf provided with -f will not be used to adjust isoform
        TSSs/TESs each isoform will be determined from supporting reads''')
    parser.add_argument('--max_ends', default=2, action='store', dest='max_ends',
                        help='maximum number of TSS/TES picked per isoform (2)')
    parser.add_argument('--trust_ends', default=False, action='store_true', dest='trust_ends',
                        help='specify if reads are generated from a long read method with minimal fragmentation')
    parser.add_argument('--filter', default='default', action='store', dest='filter', help='''Report options include:
        nosubset--any isoforms that are a proper set of another isoform are removed;
        default--subset isoforms are removed based on support;
        comprehensive--default set + all subset isoforms;
        ginormous--comprehensive set + single exon subset isoforms''')
    ...
    parser.add_argument('--range', default='', action='store', dest='range',
                        help='interval for which to collapse isoforms for, formatted chromosome:coord1-coord2 or '
                             'tab-delimited; if a range is specified, then the aligned reads bam must be specified with'
                             ' -r and the query must be a sorted, bgzip-ed bed file')
    parser.add_argument('--salmon', type=str, action='store', dest='salmon', default='',
                        help='Path to salmon executable, specify if salmon quantification is desired')
    parser.add_argument('--temp_dir', default='', action='store', dest='temp_dir',
                        help='directory to put temporary files. use "./" to indicate current directory'
    ...
    if args.range:
        ...
        for i in range(len(args.r)):  # subset bam file for alignments within range
            ...
            if subprocess.call([args.sam, 'view', '-h', args.r[i], args.range], stdout=open(bams[-1], 'w')):
                return 1
        args.r = []
        for i in range(len(bams)):  # read sequences of the alignments within range
            args.r += [bams[i][:-3] + 'fasta']
            subprocess.call([args.sam, 'fasta', bams[i]], stdout=open(args.r[-1], 'w'),
        ...
        if subprocess.call(['tabix', '-R', args.temp_dir + tempfile_name + args.range + '.range.bed', args.q],
            ...
            sys.stderr.write(
                'Query file needs to be a sorted, bgzip-ed, tabix-indexed bed file if range is specified\n')
            return 1
    else:
    ...
    if args.p:
        ...
        if subprocess.call(
                [sys.executable, path + 'bin/pull_starts.py', args.q, args.temp_dir + tempfile_name + 'tss.bed']):
            return 1
        ...
        subprocess.call([sys.executable, path + 'bin/psl_reads_from_bed.py',
                         args.temp_dir + tempfile_name + 'promoter_intersect.bed', args.q, precollapse])
    ...
    if args.threeprime:
        ...
        if subprocess.call(
                [sys.executable, path + 'bin/pull_starts.py', precollapse, args.temp_dir + tempfile_name + 'tes.bed',
                 'reverse']):
            return 1
        if subprocess.call(
                [args.b, 'intersect', '-a', args.temp_dir + tempfile_name + 'tes.bed', '-b', args.threeprime],
        ...
        subprocess.call(
            [sys.executable, path + 'bin/psl_reads_from_bed.py', args.temp_dir + tempfile_name + 'tes_intersect.bed',
             args.q, precollapse])
        intermediate += [args.temp_dir + tempfile_name + 'tes.bed', precollapse]

    collapse_cmd = [sys.executable, path + 'bin/collapse_isoforms_precise.py', '-q', precollapse, '-m',
                    str(args.max_ends), '-w', args.w, '-n', args.n, '-o', args.o + 'firstpass.unfiltered' + ext]
    ...
    if subprocess.call(
            [sys.executable, path + 'bin/filter_collapsed_isoforms.py', args.o + 'firstpass.unfiltered' + ext,
             args.filter, args.o + 'firstpass' + ext, args.w]):
        return 1
    ...
    if args.f:
        ...
        if subprocess.call([sys.executable, path + 'bin/identify_gene_isoform.py', args.o + 'firstpass' + ext, args.f,
                            args.o + 'firstpass.named' + ext]):
            sys.exit(1)
        subprocess.call(['mv', args.o + 'firstpass.named' + ext, args.o + 'firstpass' + ext])

    if subprocess.call([sys.executable, path + 'bin/psl_to_sequence.py', args.o + 'firstpass' + ext, args.g,
                        args.o + 'firstpass.fa']):
        return 1
    ...
    if args.salmon:  # use salmon to count
        ...
        subprocess.call(
            [args.salmon, 'quant', '-t', args.o + 'firstpass.fa', '-o', alignout + 'salmon', '-p', args.t, '-l', 'U',
             '-a', alignout + 'sam'],
            stderr=open(alignout + 'salmon_stderr.txt', 'w'))
        ...
        count_cmd = [sys.executable, path + 'bin/count_sam_transcripts.py', '-s', alignout + 'q.sam', '-o',
                     alignout + 'q.counts', '-t', args.t, '--quality', args.quality]
        if args.stringent:
    ...
    subprocess.call(
        [sys.executable, path + 'bin/psl_to_sequence.py', args.o + 'isoforms' + ext, args.g, args.o + 'isoforms.fa'])
    if args.f:
    ...
    if not args.keep_intermediate:
        ...
        subprocess.call(
            ['rm', '-rf'] + glob.glob(args.temp_dir + '*' + tempfile_name + '*') + align_files + intermediate)
    return args.o + 'isoforms.bed', args.o + 'isoforms.fa'
...
def quantify(isoform_sequences=''):
    ...
    if not isoform_sequences:
        required.add_argument('-r', '--reads_manifest', action='store', dest='r', type=str, required=True,
                              help='Tab delimited file containing sample id, condition, batch, reads.fq')
        required.add_argument('-i', '--isoforms', action='store', dest='i', type=str, required=True,
                              help='FastA of FLAIR collapsed isoforms')
    else:
        required.add_argument('--reads_manifest', action='store', dest='r', type=str, required=True,
                              help='Tab delimited file containing sample id, condition, batch, reads.fq')
    parser.add_argument('-m', '--minimap2', type=str, default='minimap2', action='store', dest='m',
                        help='path to minimap2 if not in $PATH')
    parser.add_argument('-t', '--threads', type=int, action='store', dest='t', default=4,
    ...
    parser.add_argument('--quality', type=int, action='store', dest='quality', default=1,
        ...
        used (1)''')
    parser.add_argument('-o', '--output', type=str, action='store', dest='o', default='counts_matrix.tsv',
                        help='Counts matrix output file name prefix (counts_matrix.tsv)')
    parser.add_argument('--salmon', type=str, action='store', dest='salmon', default='',
                        help='Path to salmon executable, specify if salmon quantification is desired')
    ...
    with codecs.open(args.r, "r", encoding='utf-8', errors='ignore') as lines:
        ...
        for num, sample in enumerate(samData, 0):
            sys.stderr.write(
                "Step 1/3. Aligning sample %s_%s: %s/%s \r" % (sample[0], sample[2], num + 1, len(samData)))
    ...
    for num, data in enumerate(samData):
        ...
        sys.stderr.write(
            "Step 2/3. Quantifying isoforms for sample %s_%s: %s/%s \r" % (sample, batch, num + 1, len(samData)))

        if not args.salmon:
            count_cmd = [sys.executable, path + 'bin/count_sam_transcripts.py', '-s', samOut, '-o',
                         samOut + '.counts.txt', '-t', args.t, '--quality', args.quality]
            if args.trust_ends:
                count_cmd += ['--trust_ends']
            ...
            subprocess.call(
                [args.salmon, 'quant', '-t', args.i, '-o', samOut[:-4] + '.salmon', '-p', args.t, '-l', 'U', '-a',
                 samOut], stderr=open('salmon_stderr.txt', 'w'))
...
def diffExp(counts_matrix=''):
    ...
    if not counts_matrix:
        required.add_argument('-q', '--counts_matrix', action='store', dest='q', type=str, required=True,
                              help='Tab-delimited isoform count matrix from flair quantify module.')
    required.add_argument('-o', '--out_dir', action='store', dest='o', type=str, required=True,
                          help='Output directory for tables and plots.')
    parser.add_argument('-t', '--threads', action='store', dest='t', type=int, required=False, default=4,
                        help='Number of threads for parallel DRIMSeq.')
    parser.add_argument('-e', '--exp_thresh', action='store', dest='e', type=int, required=False, default=10,
                        help='Read count expression threshold. Isoforms in which both conditions contain fewer than E '
                             'reads are filtered out (Default E=10)')
    parser.add_argument('-of', '--out_dir_force', action='store_true', dest='of', required=False,
                        help='Specify this argument to force overwriting of files in an existing output directory')
    args, unknown = parser.parse_known_args()
    if unknown:
    ...
    DEcommand = [sys.executable, '-W ignore', runDE, '--filter', str(args.e), '--threads', str(args.t), '--outDir',
...
def diffSplice(isoforms='', counts_matrix=''):
    parser = argparse.ArgumentParser(description='flair-diffSplice parse options',
                                     usage='python flair.py diffSplice -i isoforms.bed|isoforms.psl '
                                           '-q counts_matrix.tsv [options]')
    parser.add_argument('diffExp')
    required = parser.add_argument_group('required named arguments')
    if not isoforms:
        required.add_argument('-i', '--isoforms', action='store', dest='i', required=True, type=str,
                              help='isoforms in bed or psl format')
        required.add_argument('-q', '--counts_matrix', action='store', dest='q', type=str, required=True,
                              help='tab-delimited isoform count matrix from flair quantify module')
    ...
    parser.add_argument('--test', action='store_true', dest='test', required=False, default=False,
                        help='Run DRIMSeq statistical testing')
    parser.add_argument('-t', '--threads', action='store', dest='t', type=int, required=False, default=1,
                        help='Number of threads DRIMSeq (1)')
    parser.add_argument('--drim1', action='store', dest='drim1', type=int, required=False, default=6,
                        help='The minimum number of samples that have coverage over an AS event inclusion/exclusion for'
                             ' DRIMSeq testing; events with too few samples are filtered out and not tested (6)')
    parser.add_argument('--drim2', action='store', dest='drim2', type=int, required=False, default=3,
                        help='The minimum number of samples expressing the inclusion of an AS event; events with too '
                             'few samples are filtered out and not tested (3)')
    ...
    parser.add_argument('--drim4', action='store', dest='drim4', type=int, required=False, default=5,
                        help='The minimum number of reads covering an AS event inclusion for DRIMSeq testing, events '
                             'with too few samples are filtered out and not tested (5)')
    ...
    parser.add_argument('--conditionA', action='store', dest='conditionA', required=False, default='',
                        help='Specify one condition corresponding to samples in the counts_matrix to be compared '
                             'against condition2; by default, the first two unique conditions are used')
    parser.add_argument('--conditionB', action='store', dest='conditionB', required=False, default='',
                        help='Specify another condition corresponding to samples in the counts_matrix to be compared '
                             'against conditionA')
    args, unknown = parser.parse_known_args()
    if unknown:
    ...
    if args.test or args.drim1 or args.drim2 or args.drim4 or args.drim4:
        ...
        ds_command = [sys.executable, path + 'bin/runDS.py', '--threads', str(args.t), '--drim1', drim1, '--drim2',
                      drim2, '--drim3', drim3, '--drim4', drim4]
        if args.batch:
            ds_command += ['--batch']
        if args.conditionA:
        ...
        with open(args.o + '.stderr.txt', 'w') as ds_stderr:
            subprocess.call(ds_command + ['--matrix', args.o + '.es.events.quant.tsv', '--prefix', args.o + '.es'],
                            stderr=ds_stderr)
            subprocess.call(ds_command + ['--matrix', args.o + '.alt5.events.quant.tsv', '--prefix', args.o + '.alt5'],
                            stderr=ds_stderr)
            subprocess.call(ds_command + ['--matrix', args.o + '.alt3.events.quant.tsv', '--prefix', args.o + '.alt3'],
                            stderr=ds_stderr)
            subprocess.call(ds_command + ['--matrix', args.o + '.ir.events.quant.tsv', '--prefix', args.o + '.ir'],
                            stderr=ds_stderr)
    return
...
if mode == 'correct' or '2' in mode:
    if aligned_reads:
        status = parse_correct(aligned_reads=aligned_reads)
    else:
        status = parse_correct()
    if status == 1:
        sys.exit(1)
    else:
...
if mode == 'collapse-range' or '3.5' in mode:
    ...
    if corrected_reads and not aligned_reads:
        ...
        status = collapse_range(corrected_reads=corrected_reads, aligned_reads=corrected_reads[:-18] + '.bam')
